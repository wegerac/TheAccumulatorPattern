...
def draw_squares_from_circle(n, circle, window):
    ...
    """
    # ------------------------------------------------------------------
    # DONE34tc : 2. Implement and test this function.
    ...
    for k in range(n):
        square = rg.Square(circle.center, circle.radius * 2)
        square.move_by(circle.radius * k, circle.radius * k)
        square.attach_to(window)
    window.render()
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    circ_row_rad = rectangle.get_height() / 2
    circ_col_rad = rectangle.get_width() / 2
    center1 = rg.Point(rectangle.get_center().x - circ_col_rad - circ_row_rad, rectangle.get_center().y)
    circle1 = rg.Circle(center1, circ_row_rad)
    circle1.fill_color = rectangle.fill_color
    circle1.attach_to(window)
    for k in range(m - 1):
        center1 = rg.Point(rectangle.get_center().x - circ_col_rad - circ_row_rad - (2 * circ_row_rad * (k + 1)),
                           rectangle.get_center().y)
        circle1 = rg.Circle(center1, circ_row_rad)
        circle1.fill_color = rectangle.fill_color
        circle1.attach_to(window)
    center2 = rg.Point(rectangle.get_center().x, rectangle.get_center().y - circ_col_rad - circ_row_rad)
    circle2 = rg.Circle(center2, circ_col_rad)
    circle2.outline_color = rectangle.outline_color
    circle2.attach_to(window)
    for k in range(n - 1):
        center2 = rg.Point(rectangle.get_center().x,
                           rectangle.get_center().y - circ_col_rad - circ_row_rad - (2 * circ_col_rad * (k + 1)))
        circle2 = rg.Circle(center2, circ_col_rad)
        circle2.outline_color = rectangle.outline_color
        circle2.attach_to(window)
